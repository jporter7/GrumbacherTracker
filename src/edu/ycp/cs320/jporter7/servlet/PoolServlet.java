...
public class PoolServlet extends HttpServlet {
			...
			throws ServletException, IOException 
	{
		...
		test2 = db.getReservationsForRoom("3", today);
		for (int i = 0; i < test2.size(); i++)
		{
			users.add(db.getUserFromReservationTime("3", test2.get(i).getStartTime()));
		}
		...
		else
		{
			...
			req.getRequestDispatcher("/_view/pool.jsp").forward(req, resp);
		}
	...
	public void makeReservationForTime(String time)
	{
		...
		{
			Reservation reservation = new Reservation(today, time, user2.getDbId(), 3);
			userWithReservation = null;
			userWithReservation = db.getUserFromReservationTime("3", time);
			...
			if(user2.getIsFaculty() && userWithReservation != null)
			{
				...
				{
					db.removeReservation("3", time);
					db.insertReservation(reservation.getDate(), reservation.getStartTime(), 3, user2.getDbId());
