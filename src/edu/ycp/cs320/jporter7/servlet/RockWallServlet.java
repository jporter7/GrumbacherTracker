...
public class RockWallServlet extends HttpServlet {
			...
			throws ServletException, IOException 
	{
		...
		test2 = db.getReservationsForRoom("5", today);
		for (int i = 0; i < test2.size(); i++)
		{
			users.add(db.getUserFromReservationTime("5", test2.get(i).getStartTime()));
		}
	...
	public void makeReservationForTime(String time)
	{
		...
		{
			Reservation reservation = new Reservation(today, time, user2.getDbId(), 5);
			userWithReservation = null;
			userWithReservation = db.getUserFromReservationTime("5", time);
			...
			if(user2.getIsFaculty() && userWithReservation != null)
			{
				...
				if (userWithReservation.getIsFaculty() == false)
				{
					...
					db.insertReservation(reservation.getDate(), reservation.getStartTime(), 5, user2.getDbId());
