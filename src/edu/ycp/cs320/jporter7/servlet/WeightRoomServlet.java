...
public class WeightRoomServlet extends HttpServlet {
			...
			throws ServletException, IOException 
	{
		...
		test2 = db.getReservationsForRoom("6", today);
		for (int i = 0; i < test2.size(); i++)
		{
			users.add(db.getUserFromReservationTime("6", test2.get(i).getStartTime()));
		}
	...
	public void makeReservationForTime(String time)
	{
		...
		{
			Reservation reservation = new Reservation(today, time, user2.getDbId(), 6);
			userWithReservation = null;
			userWithReservation = db.getUserFromReservationTime("6", time);
			...
			if(user2.getIsFaculty() && userWithReservation != null)
			{
				...
				{
					db.removeReservation("6", time);
					db.insertReservation(reservation.getDate(), reservation.getStartTime(), 6, user2.getDbId());
